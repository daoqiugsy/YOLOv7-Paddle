...
class IDetect(nn.Layer):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [paddle.zeros(1)] * self.nl  # init grid
        a = paddle.to_tensor(anchors).float().view(self.nl, -1, 2)
        self.register_buffer('anchors', a)  # shape(nl,na,2)
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        self.m = nn.LayerList(nn.Conv2D(x, self.no * self.na, 1) for x in ch)  # output conv

        self.ia = nn.LayerList(ImplicitA(x) for x in ch)
        self.im = nn.LayerList(ImplicitM(self.no * self.na) for _ in ch)
...
def parse_model(d, ch, mode='training'):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, layer, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost,RepConv,SPPCSPC]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, SPPCSPC]:
